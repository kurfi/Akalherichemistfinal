            ...
            <div className="flex-1 flex flex-col bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden no-print">
                <div className="p-4 border-b border-slate-200 flex gap-2">
                    <div className="relative flex-1">
                        ...
                        <input
                            className="w-full pl-10 pr-4 py-2 rounded-lg bg-slate-50 border-none focus:ring-2 focus:ring-emerald-500 outline-none transition-all"
                ...
                <div className="flex-1 overflow-y-auto p-4 bg-slate-50">
                    <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
                        {productsWithStock?.map(product => {
                            ...
                            const statusLabel = product.isOutOfStock ? (product.hasExpiredStock ? 'Expired' : 'Out of Stock') : null;

                            return (
                                <div
                                    key={product.id}
                                    onClick={() => !isDisabled && addToCart(product)}
                                    className={`bg-white p-4 rounded-lg border shadow-sm transition-all group flex flex-col justify-between h-40 ${isDisabled
                                        ? 'opacity-50 grayscale cursor-not-allowed border-slate-200'
                                        : 'hover:shadow-md hover:border-emerald-400 cursor-pointer border-slate-200'
                                        }`}
                                >
                                    <div>
                                        <div className="flex justify-between items-start">
                                            ...
                                            {statusLabel && (
                                                <span className={`text-[10px] px-1.5 py-0.5 rounded font-bold uppercase ${product.hasExpiredStock ? 'bg-red-100 text-red-600' : 'bg-slate-200 text-slate-600'
                                                    }`}>
                                                    {statusLabel}
                                                </span>
                                            )}
                                        ...
                                        <div className="mt-2 flex items-center gap-1.5">
                                            ...
                                            <span className={`text-xs font-medium ${product.validStock <= product.minStockLevel ? 'text-amber-600' : 'text-slate-600'}`}>
                                                Stock: {product.validStock}
                                            </span>
                                        </div>
                                    </div>
                                    <div className="flex justify-between items-end mt-2">
                                        <span className="font-bold text-emerald-600">₦{product.price.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                        <div className={`p-1 rounded transition-colors ${isDisabled
      ...
      `}>
                ...
                {activeTab === 'cart' && (
                    <>
                                ...
                                cart?.map(item => (
                                    <div key={item.productId} className="flex justify-between items-center p-3 bg-slate-50 rounded-lg border border-slate-100">
                                        <div className="flex-1 min-w-0 pr-2">
                                            <h4 className="font-medium text-slate-800 truncate text-sm">{item.productName}</h4>
                                            <p className="text-xs text-slate-500">₦{item.price.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} / unit</p>
                                        </div>
                                        ...
                                        <div className="ml-3 text-right min-w-[60px]">
                                            <p className="font-bold text-slate-800 text-sm">₦{item.total.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                                            <button onClick={() => removeFromCart(item.productId)} className="text-xs text-red-400 hover:text-red-600 mt-1">Remove</button>
                                        </div>
                                    </div>
                                ))
                            )}
                        ...
                        <div className="p-6 bg-slate-900 text-white rounded-b-xl mt-auto">
                            ...
                            <div className="flex justify-between items-center mb-6 text-sm text-slate-300">
                                ...
                                <input
                                    ...
                                    className="w-24 px-2 py-1 rounded bg-slate-800 border border-slate-700 text-right text-white focus:ring-2 focus:ring-emerald-500 outline-none"
                                    placeholder="0.00"
                                />
                            </div>
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                <button
                                    ...
                                    className="col-span-1 py-3 bg-amber-600 hover:bg-amber-700 text-white rounded-lg font-bold transition-colors disabled:opacity-50 flex items-center justify-center gap-1"
                                >
                                ...
                                <button
                                    ...
                                    disabled={cart.length === 0}
                                    className="col-span-2 py-3 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg font-bold transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
                                >
            ...
            {isPaymentModalOpen && (
                <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4 no-print">
                    <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl p-6">
                        ...
                        <div className="text-center mb-8 bg-slate-50 p-4 rounded-xl">
                            <p className="text-slate-500 text-sm uppercase tracking-wide mb-1">Total Payable</p>
                            <p className="text-4xl font-extrabold text-emerald-600">₦{Math.max(0, calculateTotal() - discount).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                            {discount > 0 && (
                                <p className="text-sm text-slate-400 mt-1 line-through">₦{calculateTotal().toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                            )}
                        </div>

                        {!isMultipayOpen ? (
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
                                <button onClick={() => handleCheckout(PaymentMethod.CASH)} className="group p-6 border border-slate-200 rounded-xl hover:bg-emerald-50 hover:border-emerald-500 hover:shadow-md transition-all flex flex-col items-center gap-3">
                                    <div className="p-3 bg-emerald-100 text-emerald-600 rounded-full group-hover:bg-emerald-200"><Banknote className="w-6 h-6" /></div>
                                    <span className="font-bold text-slate-700 group-hover:text-emerald-700">Cash</span>
                                </button>
                                <button onClick={() => handleCheckout(PaymentMethod.CARD)} className="group p-6 border border-slate-200 rounded-xl hover:bg-blue-50 hover:border-blue-500 hover:shadow-md transition-all flex flex-col items-center gap-3">
                                    <div className="p-3 bg-blue-100 text-blue-600 rounded-full group-hover:bg-blue-200"><CreditCard className="w-6 h-6" /></div>
                                    <span className="font-bold text-slate-700 group-hover:text-blue-700">POS / Card</span>
                                </button>
                                <button onClick={() => handleCheckout(PaymentMethod.TRANSFER)} className="group p-6 border border-slate-200 rounded-xl hover:bg-purple-50 hover:border-purple-500 hover:shadow-md transition-all flex flex-col items-center gap-3">
                                    <div className="p-3 bg-purple-100 text-purple-600 rounded-full group-hover:bg-purple-200"><Smartphone className="w-6 h-6" /></div>
                                    <span className="font-bold text-slate-700 group-hover:text-purple-700">Transfer</span>
                                </button>
                                <button onClick={() => setIsMultipayOpen(true)} className="group p-6 border border-slate-200 rounded-xl hover:bg-fuchsia-50 hover:border-fuchsia-500 hover:shadow-md transition-all flex flex-col items-center gap-3">
                                    <div className="p-3 bg-fuchsia-100 text-fuchsia-600 rounded-full group-hover:bg-fuchsia-200"><Layers className="w-6 h-6" /></div>
                                    <span className="font-bold text-slate-700 group-hover:text-fuchsia-700">Multipay</span>
                                </button>
                                <button onClick={() => handleCheckout(PaymentMethod.CREDIT)} disabled={!selectedCustomer} className={`group p-6 border border-slate-200 rounded-xl transition-all flex flex-col items-center gap-3 ${!selectedCustomer ? 'opacity-50 cursor-not-allowed bg-slate-50' : 'hover:bg-orange-50 hover:border-orange-500 hover:shadow-md cursor-pointer'}`}>
                                    <div className={`p-3 rounded-full ${!selectedCustomer ? 'bg-slate-200 text-slate-400' : 'bg-orange-100 text-orange-600 group-hover:bg-orange-200'}`}><FileText className="w-6 h-6" /></div>
                                    <span className={`font-bold ${!selectedCustomer ? 'text-slate-400' : 'text-slate-700 group-hover:text-orange-700'}`}>Debt / Credit</span>
                                    {!selectedCustomer && <span className="text-[10px] text-red-400 -mt-2">(Select Customer)</span>}
                                </button>
                            </div>
                        ) : (
                            <div className="space-y-4">
                                <form onSubmit={(e) => {
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <input name="amount" type="number" placeholder="Amount" className="flex-1 p-2 rounded border border-slate-300 bg-white text-sm focus:ring-2 focus:ring-emerald-500 outline-none" />
                                        <button type="submit" className="p-2 bg-emerald-500 text-white rounded hover:bg-emerald-600"><Plus className="w-5 h-5" /></button>
                                    </div>
                                </form>
                                <div className="space-y-2">
                                    {multipayEntries?.map((entry, index) => (
                                        <div key={index} className="flex justify-between items-center p-2 bg-slate-100 rounded">
                                            <span className="text-sm">{entry.method}: ₦{parseFloat(entry.amount).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                            <button onClick={() => setMultipayEntries(multipayEntries.filter((_, i) => i !== index))} className="p-1 text-red-500 hover:bg-red-100 rounded"><X className="w-4 h-4" /></button>
                                        </div>
                                    ))}
                                </div>
                                <div className="pt-4 border-t border-slate-200 space-y-2 text-sm">
                                    <div className="flex justify-between">
                                        ...
                                        <span className="font-medium">₦{multipayEntries.reduce((acc, e) => acc + parseFloat(e.amount || '0'), 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-slate-500">Remaining:</span>
                                        <span className="font-medium">₦{(Math.max(0, calculateTotal() - discount) - multipayEntries.reduce((acc, e) => acc + parseFloat(e.amount || '0'), 0)).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                    </div>
                                </div>
                                <button
                                    ...
                                    className="w-full py-3 bg-emerald-500 text-white rounded-lg font-bold transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
                                >
                                    Confirm Payment
                                </button>
                            </div>
            ...
            {/* Receipt Modal */}
            {receiptSale && (
                <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" id="printable-receipt">
                    <div className="bg-white rounded-xl shadow-2xl flex flex-col">
                        <div className="p-4 border-b border-slate-200 flex justify-between items-center bg-slate-50 no-print">
                            <div className="flex items-center gap-2">
                                <button onClick={() => setPaperSize('80mm')} className={`px-2 py-1 text-xs rounded ${paperSize === '80mm' ? 'bg-slate-800 text-white' : 'bg-slate-200 text-slate-600'}`}>80mm</button>
                                <button onClick={() => setPaperSize('58mm')} className={`px-2 py-1 text-xs rounded ${paperSize === '58mm' ? 'bg-slate-800 text-white' : 'bg-slate-200 text-slate-600'}`}>58mm</button>
                            </div>
                            <button onClick={() => setReceiptSale(null)} className="p-2 hover:bg-slate-200 rounded-full transition-colors">
                        ...
                        <div className="p-4 border-t border-slate-200 flex gap-3 justify-end bg-white no-print">
                            <button
                            ...
                            >
                                <Printer className="w-4 h-4" /> Print Thermal (Raw)
                            </button>
                            <button
